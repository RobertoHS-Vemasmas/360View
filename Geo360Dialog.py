...
# Codigo optimizado (puede mejorar)
# import math
# import os
...
#             self.ChangeUrlViewer(self.DEFAULT_URL)
#         else:
#             qgsutils.showUserAndLogMessage("Información: ", "No existe imagen asociada o la ruta es inválida")
#             self.resetQgsRubberBand()
#             self.ChangeUrlViewer(self.DEFAULT_EMPTY)
#             return
...
#     def CreateViewer(self):
#         qgsutils.showUserAndLogMessage("Información: ", "Crear visor", onlyLog=True)
#         self.cef_widget = QWebView()
#         self.cef_widget.setContextMenuPolicy(Qt.NoContextMenu)
...
#             error_url = reply.url().toString()
#             error_message = reply.errorString()
#             qgsutils.showUserAndLogMessage(f"Error: Error al acceder a {error_url}, Código: {error_code}, Mensaje: {error_message}")
#             qgsutils.showUserAndLogMessage("Error: ", reply.errorString())
...
#             self.path = f'https://10.16.106.74/geo/360/{punto["zona"]}/{punto["recorrido"]}/{imagenNombre}'
#             self.current_image = self.path

#             qgsutils.showUserAndLogMessage("Información: ", str(self.path), onlyLog=True)
#         else:
#             qgsutils.showUserAndLogMessage("Error: ", reply.errorString())
...
#         if not os.path.exists(self.current_image):
#             qgsutils.showUserAndLogMessage("Información: ", "No existe imagen asociada.")
#             self.ChangeUrlViewer(self.DEFAULT_EMPTY)
#             self.resetQgsRubberBand()
#             return
...
#         if not lys:
#             qgsutils.showUserAndLogMessage("Información: ", "Necesita cargar la capa de fotos.")
#             return
...
#                 if not ids:
#                     qgsutils.showUserAndLogMessage("Información: ", "Sin imagen.")
#                     ids = [
#                         feat.id()
#                         for feat in layer.getFeatures(
#                             QgsFeatureRequest().setFilterExpression(
...
#         if not self.found:
#             qgsutils.showUserAndLogMessage(
#                 "Información: ", "Necesita una capa con imágenes y establecer el nombre en el archivo config.py"
#             )
...
#             if self.positionDx:
#                 self.positionDx.reset()
#             if self.actualPointOrientation:
#                 self.actualPointOrientation.reset()
#         except Exception:
#             pass
...
#         self.x = round(self.x, 5)
#         self.y = round(self.y, 5)
#         json_data = {'Latitud': self.y, 'Longitud': self.x}  # Se invierten las coordenadas
